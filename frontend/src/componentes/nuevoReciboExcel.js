...
function NuevoReciboExcel() {
  ...
  useEffect(() => {
    if (selectedCondominio) {
      axios
        .post(`${API_URL}/api/getEdificiosbyCondominio`, {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const checkInquilinosPromises = deptosBD.map((depto) =>
        axios.post(`${API_URL}/api/getInquilinosbyDepartamento`, {
      ...
      for (let fila of datos) {
        ...
        if (fila[10]) {
          ...
          const response = await axios.get(
            `${API_URL}/api/verificarRecibo/${selectedCondominio}/${no_recibo}`
          );
      ...
      for (let fila of datos) {
        ...
        if (fila[7] === undefined || fila[7] === "") {
          ...
          const inquilinos = await axios
            .post(`${API_URL}/api/getInquilinosbyDepartamento`, {
              id_departamento: deptoEncontrado.id_departamento,
            })
            .then((res) => res.data);
      ...
      await axios.post(`${API_URL}/api/registrarRecibo`, datosRecibo);
      await axios.post(
        `${API_URL}/api/registrarInfoPagosCompleto`,
        datosInfoPagos
      );
