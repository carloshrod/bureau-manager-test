...
function VerRecibo() {
  ...
  const cargarCondominios = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/getCondominios/${id_administrador}`
      );
  ...
  const cargarEdificios = async (idCondominio) => {
    try {
      const response = await axios.post(
        `${API_URL}/api/getEdificiosbyCondominio`,
  ...
  const cargarRecibos = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/api/getRecibos/${id_administrador}`
      );
    ...
    if (buttonValue === "correo") {
      try {
        const resultado = await axios.post(
          `${API_URL}/api/enviarRecibosCorreoElectronico`,
  ...
  const handleFiltrar = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/api/getRecibosFiltrados/${id_administrador}`,
        {
          params: {
  ...
  const handleEliminarRecibos = async () => {
    ...
    if (
      ...
      try {
        await axios.post(`${API_URL}/api/eliminarRecibos`, {
